...
const Backdrop = () => {
  useGSAP(() => {
    ...
    // animate all paths stroke from 0 to 1
    tl.fromTo(
      "path",
      { strokeDashoffset: 0 },
  ...
  useEffect(() => {
    if (svgRef.current && innerHeight && innerWidth) {
      // if mobile
      if (innerWidth < 768) {
        svgRef.current.style.transform = `translateX(-${innerWidth / 2}px)`
      } else {
        svgRef.current.style.transform = `translateX(0px)`
      }
    }
  }, [innerHeight, innerWidth])

  return (
    <div className="absolute top-0 left-0 h-dvh w-full overflow-hidden object-cover z-[-1]">
      <svg
        ref={svgRef}
